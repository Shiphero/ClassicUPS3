from ClassicUPS3.ups import Shipment, TrackingInfo, UPSConnection, UPSResult
...
def test_upsconnection_transmit_request_uses_production_urls_when_debug_false(
    mock_urlopen_factory, ups_connection_params
):
...
def test_upsconnection_create_shipment_convenience_method(
    mock_urlopen_factory, ups_connection_params, sample_addresses, sample_package
):
    """Test create_shipment convenience method returns Shipment instance"""
    conn = UPSConnection(**ups_connection_params)
    mock_urlopen_factory(
        response_mapping={b"ShipConfirm": "ship_confirm_response.xml", b"ShipAccept": "ship_accept_response.xml"}
    )

    shipment = conn.create_shipment(
        sample_addresses["us_from"], sample_addresses["us_to"], sample_package["dimensions"], sample_package["weight"]
    )
...
def test_upsresult_dict_response_property():
    ...
    test_xml = b"""<?xml version="1.0"?>
            <TrackResponse><Shipment><Package><Activity>
                <Status><StatusType><Code>I</Code></StatusType></Status>
                <Date>20230520</Date></Activity></Package></Shipment>
            </TrackResponse>
        """
    result = UPSResult(test_xml)
